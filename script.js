...
function processCSV(contents) {
    ...
    lines.forEach(line => {
        const [number, name] = line.trim().split(','); // 쉼표를 기준으로 학생 번호와 이름 분리
        if (number && name) { // 학생 번호와 이름이 모두 있는 경우에만 추가
...
function showCard() {
    if (currentCardIndex < students.length) {
        ...
        studentNumberSpan.style.fontSize = `${viewportWidth}px`; // 학생 번호 크기를 설정합니다.
        //question 가운데 정렬하기
        question.classList.add('question');
        ...
        question.addEventListener('click', () => {
            ...
            answer.style.fontSize = `${viewportWidth}px`; // 학생 번호 크기를 설정합니다.
